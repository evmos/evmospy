...
FOLDER=$(pwd)
I=$(PWD)/proto
DEST=$(PWD)/evmospy/evmosproto/
mkdir -p $DEST

python -m grpc_tools.protoc -I=$I --python_out=$DEST --grpc_python_out=$DEST $(find $(PWD)/proto -iname "*.proto")

# Fix imports
find $DEST -type f -a \( -name "*.py" \) -a -exec sed -i '' -e s/"from "/"from evmospy.evmosproto."/g {} +
...
find $DEST -type f -a \( -name "*.py" \) -a -exec sed -i '' -e s/"from evmospy.evmosproto.google.protobuf.internal import enum_type_wrapper"/"from google.protobuf.internal import enum_type_wrapper"/g {} +
find $DEST -type f -a \( -name "*.py" \) -a -exec sed -i '' -e s/"from evmospy.evmosproto.google.protobuf import timestamp_pb2 as google_dot_protobuf_dot_timestamp__pb2"/"from google.protobuf import timestamp_pb2 as google_dot_protobuf_dot_timestamp__pb2"/g {} +
find $DEST -type f -a \( -name "*.py" \) -a -exec sed -i '' -e s/"from evmospy.evmosproto.google.protobuf import duration_pb2 as google_dot_protobuf_dot_duration__pb2"/"from google.protobuf import duration_pb2 as google_dot_protobuf_dot_duration__pb2"/g {} +
